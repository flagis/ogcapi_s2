...
function landingPageHTML() {
    var tmpl = swig.compileFile(__dirname + '/templates/landingPage.template'),
    renderedHtml = tmpl({
        title: serviceTitle,
        url: serviceUrl,
    });
...
function collectionsHTML(collections) {
    ...
    var tmpl = swig.compileFile(__dirname + '/templates/collections.template'),
    renderedHtml = tmpl({
        collections: items,
    });
...
function collectionHTML(collectionId) {
    ...
    var tmpl = swig.compileFile(__dirname + '/templates/collection.template'),
    renderedHtml = tmpl({
        collection: item,
    });
...
function itemsHTML(collectionId, geojson) {
    ...
    var tmpl = swig.compileFile(__dirname + '/templates/items.template'),
    renderedHtml = tmpl({
        collection: item,
    });
